...
In our case, **we'll split the sentence such that the long sentences are in the
test split.** Similar to [Sogaard et al's (2021)](#sogaard2021random) approach,
we'll split the dataset such that $$10\%$$ of the documents are found in the
test set. After training a transition-based NER, the results are as follows:
