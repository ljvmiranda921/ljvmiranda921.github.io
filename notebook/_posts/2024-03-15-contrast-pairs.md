...
The best way to think about these value is in terms of *chance*, that is, how likely is a random preference pair have a distance $$\mathbf{d}$$ on the x-axis?
...
I'm not familiar how exactly they prompted the LLM, but does that mean their process resulted to similar-looking texts?
...
Because Nectar formats its preferences in terms of ranking, the chosen and rejected pairs here represents the top and last choices.
...
For example, OpenAI's summarization dataset should still have closer preference pairs because of the inherent nature of the task.
