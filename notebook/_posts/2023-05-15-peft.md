...
learning via *hard prompts* (Brown et al., 2020). Take this prompt, for example, in 
...
how we constructed our prompt. Text is infinitely combinatorial: it will take a
lot of time to find the right "incantation" so that our LLM performs optimally.
This begs the question: what if we can learn our prompts?

Prompt and prefix tuning solves this by making use of *soft prompts*&mdash; a
trainable vector attached to the input embedding that is optimized via
backpropagation (while keeping the pretrained LM frozen). These two ideas
appeared at the same time but have similar mechanisms:
