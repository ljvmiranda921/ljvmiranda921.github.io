...
or connecting the adapters in parallel ([He et al., 2022](#he2021parallel)).


However, I'm more interested in combining adapters together to solve
multi-domain problems. For example, [Chronopoulou et al.
(2023)](#chronopoulou2023adaptersoup) proposed AdapterSoup, where they trained
an adapter for each domain and compute for its weight-space average at test time. 
<!-- talk about different aggregation methods: model soups, mixture of experts?-->

<!--
There are still open problems I foresee with adapters. One is the complexity-efficiency tradeoff.
It's possible to "go crazy" in architecting adapters that the efficiency gain may not worth the
complexity of the set-up. Just like in engineering, it's possible to go full-kubernetes
when you can just use a simple VM (or a cloud function). 
-->
